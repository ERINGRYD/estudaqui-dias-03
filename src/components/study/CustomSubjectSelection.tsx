...
const CustomSubjectSelection = ({ 
  ...
  const commonSubjects = [
    { name: 'Matemática', category: 'Exatas' },
    { name: 'Português', category: 'Linguagens' },
    { name: 'História', category: 'Humanas' },
    { name: 'Geografia', category: 'Humanas' },
    { name: 'Física', category: 'Exatas' },
    { name: 'Química', category: 'Exatas' },
    { name: 'Biologia', category: 'Natureza' },
    { name: 'Literatura', category: 'Linguagens' },
    { name: 'Inglês', category: 'Linguagens' },
    { name: 'Filosofia', category: 'Humanas' },
    { name: 'Sociologia', category: 'Humanas' },
    { name: 'Redação', category: 'Linguagens' },
    { name: 'Informática', category: 'Técnica' },
    { name: 'Direito Constitucional', category: 'Direito' },
    { name: 'Direito Administrativo', category: 'Direito' },
    { name: 'Direito Civil', category: 'Direito' },
    { name: 'Direito Penal', category: 'Direito' },
    { name: 'Raciocínio Lógico', category: 'Exatas' },
    { name: 'Conhecimentos Gerais', category: 'Gerais' }
  ];
            ...
            <div key={category} className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {subjects.map(subject => (
                  <Badge
                    key={subject.name}
                    variant={selectedSubjects.find(s => s.name === subject.name) ? "default" : "secondary"}
                    className="cursor-pointer hover:bg-study-primary hover:text-study-primary-foreground"
                    onClick={() => addCommonSubject(subject.name)}
                  >
                    {subject.name}
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-4">
              {selectedSubjects.map(subject => (
                <Card key={subject.id} className="p-4 bg-study-info border border-study-primary/20">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      {editingSubject === subject.id ? (
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          />
                          <span 
                            className="flex-1 cursor-pointer font-medium text-study-primary"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          />
                          <div className="flex">
                            {Array.from({ length: subject.priority || 1 }).map((_, i) => (
                      ...
                      <div className="space-y-2">
                        <Label className="text-sm">Peso no Plano</Label>
                        <Select
                          value={subject.weight?.toString() || '2'}
                          onValueChange={(value) => updateSubject(subject.id, { weight: parseInt(value) })}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="1">Baixo (1h/dia)</SelectItem>
                            <SelectItem value="2">Médio (2h/dia)</SelectItem>
                            <SelectItem value="3">Alto (3h/dia)</SelectItem>
                            <SelectItem value="4">Muito Alto (4h+/dia)</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                    </div>
