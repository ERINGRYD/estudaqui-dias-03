      ...
      <Card className="lg:col-span-2">
        <CardContent className="pt-6">
          <div className="text-center">
            ...
            <div className="mb-8">
              <Progress 
                value={timerMode === 'study' 
                  ? ((pomodoroSettings.studyTime - timer) / pomodoroSettings.studyTime) * 100
                  : ((pomodoroSettings.breakTime - timer) / pomodoroSettings.breakTime) * 100
                } 
                className="h-4"
              />
            </div>
              ...
              {!isTimerRunning && timer === 0 ? (
                <div className="w-full">
                  <HierarchicalTopicSelector 
                    subjects={subjects}
                    onStartSession={onStartTimer}
                  />
                </div>
              ) : (
                <div className="flex flex-wrap justify-center gap-4">
                  {isTimerRunning ? (
                    <Button onClick={onPauseTimer} size="lg" variant="outline">
                      ⏸️ Pausar
                    </Button>
                  ) : (
                    <Button onClick={onResumeTimer} size="lg" className="bg-study-primary hover:bg-study-primary/90">
                      ▶️ Continuar
                    </Button>
                  )}
